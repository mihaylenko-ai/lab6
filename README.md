...
- Server1 - Маршрутизатор имитирующий роль сервера, подключенный к Leaf коммутатору №2. Принадлежит VLAN30.
...
|3|Server1|eth1|192.168.30.10|24|
...
В качестве алгоритма IRB будем использовать Симметричную (Symmetric) маршрутизацию, позволяющую используя удиный L3VNI получить полноценную связность между раздичными L2 Bridge
доменами. На маршрутизаторах L1 и L3, была проведена настройка IRB, включающая в себя настройку отдельной VRF таблицы (дадим ей имя VRF1) а так же отдельного l3vni (дадим ему 
номер 111111), который будет един на всех тестовых Bridge доменах.
...
**Получение ECHO ICMP ответа от устройств PC10 и PC20, а так же устройства Server1**

![S1](images/PC10toPC20.jpg)

![S1](images/Server1toPC10PC20.jpg)

Как видим каждое из конечных устройств получилро доступ к своему сетевому сегменту L3, что подтверждает связность посредством L3VNI. 
